#!/usr/bin/env python3
"""
CUDA Max Element Performance Analysis
Generates plots from benchmark results showing GPU performance characteristics for finding maximum element
"""
...
def load_all_csv_files(data_folder):
    ...
    csv_files = glob.glob(os.path.join(data_folder, "max_element_results*.csv"))

    if not csv_files:
        print(f"No max element CSV files found in {data_folder}")
        return None
            ...
            .agg(
                {
                    ...
                    "GPU_Result": "first",  # Max should be the same
                    "CPU_Result": "first",  # Max should be the same
...
def plot_gpu_time_vs_array_size(df):
    ...
    plt.title(
        "GPU Max Element Performance: Execution Time vs Array Size\nfor Different Block Sizes",
        fontsize=14,
    )
...
def plot_efficiency_vs_block_size(df):
    """Plot 2: GPU Efficiency vs Block Size for different array sizes"""
    plt.figure(figsize=(12, 8))
    ...
    plt.title(
        "GPU Max Element Efficiency: Speedup vs Block Size\nfor Different Array Sizes",
        fontsize=14,
    )
    plt.legend(bbox_to_anchor=(1.05, 1), loc="upper left")
    plt.grid(True, alpha=0.3)
...
def main():
    ...
    print("CUDA Max Element Performance Analysis")
    print("=" * 50)
    ...
    plot1_path = os.path.join(plots_folder, "max_element_gpu_time_vs_array_size.png")
    ...
    plot2_path = os.path.join(plots_folder, "max_element_efficiency_vs_block_size.png")
