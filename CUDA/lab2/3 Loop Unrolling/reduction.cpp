...
// A struct to hold the results of a benchmark run
...
int main(int argc, char *argv[])
{
    ...
    csv_file.open("reduction_loopunrolling_benchmark.csv");
    if (!csv_file.is_open()) {
    ...
    for (unsigned int size : sizes_to_test) {
        ...
        // Note: The original kernel reduces to a single block's worth of partial sums,
        // but the final result is expected in the first element.
    ...
    csv_file.close();
    printf("\nBenchmark results saved to reduction_loopunrolling_benchmark.csv\n");
...
void run_reduction(void (*reduce_func)(float*, float*, int, int),
                   float *d_outPtr, float *d_inPtr, int size, int n_threads)
{
    // This is a simple wrapper for a single kernel call
                              ...
                              float *d_outPtr, float *d_inPtr, int size)
{
    ...
    // Warm-up: Perform one reduction to prepare caches, etc.
    ...
    for (int i = 0; i < test_iter; i++) {
        // Since this is an in-place reduction (d_outPtr, d_outPtr), we must
        // copy the original data from d_inPtr before each timed iteration.
        cudaMemcpy(d_outPtr, d_inPtr, size * sizeof(float), cudaMemcpyDeviceToDevice);
        run_reduction(reduce_func, d_outPtr, d_outPtr, size, num_threads);
    }
